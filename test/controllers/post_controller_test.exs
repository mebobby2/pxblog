defmodule Pxblog.PostControllerTest do
  ...
  test "when logged in as an admin, shows chosen resource with author flag set to true", %{conn: conn, user: user, post: post, admin: admin} do
  ...
  test "when logged in as different user, shows chosen resource with author flag set to false", %{conn: conn, user: user, other_user: other_user, post: post} do
  ...
  test "redirects when the specified user does not exist", %{conn: conn} do
    ...
    assert get_flash(conn, :error) == "invalid_user!"
